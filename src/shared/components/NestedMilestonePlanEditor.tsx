  ...
  return (
    <section className="space-y-4 border-t border-border pt-7">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        {!readOnly && <button type="button" onClick={() => { onChange([...value, newMilestone(value.length)]); setExpanded(value.length); }} className="inline-flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-sm font-semibold hover:bg-muted"><Plus size={16} /> {uiCopy.addMilestone || 'Add milestone'}</button>}
      </div>
      ...
      {value.length === 0 ? (
        <div className="rounded-lg border border-dashed border-border p-8 text-center">
          ...
          {!readOnly && <button type="button" onClick={() => { onChange([newMilestone(0)]); setExpanded(0); }} className="mt-4 inline-flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-sm font-semibold"><Plus size={16} /> {uiCopy.addFirstMilestone || 'Add first milestone'}</button>}
        </div>
          ...
          <article key={milestone.id || index} className={`overflow-hidden rounded-lg border bg-card ${Object.keys(errors).some(key => key.startsWith(`${index}.`)) ? 'border-red-500/60' : 'border-border'}`}>
            <div className="flex items-center gap-2 p-3 sm:p-4">
              <button type="button" onClick={() => setExpanded(isExpanded ? null : index)} aria-expanded={isExpanded} className="flex min-w-0 flex-1 items-center gap-3 text-left">
                ...
                <span className="min-w-0 flex-1"><strong className="block truncate text-sm">{milestone.title?.trim() || `${uiCopy.untitledMilestone || 'Untitled milestone'} ${index + 1}`}</strong><span className="text-xs text-muted-foreground">{formatGigCoin(Number(milestone.amount) || 0)} · {milestone.workItems.length} {uiCopy.workItems || 'work item(s)'}</span></span>
              </button>
              {!readOnly && <div className="flex shrink-0 gap-1">
                ...
                <button type="button" title={uiCopy.deleteMilestone || 'Delete milestone'} onClick={() => onChange(normalize(value.filter((_, itemIndex) => itemIndex !== index)))} className="rounded p-2 text-red-500 hover:bg-red-500/10"><Trash2 size={15} /></button>
              </div>}
            </div>
            ...
            {isExpanded && <div className="space-y-5 border-t border-border bg-background/40 p-4">
              ...
              <div className="space-y-3 rounded-lg border border-border bg-card p-3">
                ...
                {milestone.workItems.map((workItem, workIndex) => <div key={workItem.id || workIndex} className="grid gap-2 rounded-lg border border-border p-3 md:grid-cols-2">
                  ...
                  <label className="text-xs font-semibold">{uiCopy.workItemTitle || 'Work item title'}<input disabled={readOnly} maxLength={workItemTitleMaxLength} value={workItem.title || ''} onChange={e => updateWorkItem(index, workIndex, { title: e.target.value })} placeholder={fieldPlaceholders.workItemTitle || 'Work item title'} aria-label={uiCopy.workItemTitle ? `${uiCopy.workItem || 'Work item'} ${workIndex + 1}: ${uiCopy.workItemTitle}` : `Work item ${workIndex + 1} title`} aria-describedby={describedBy(`${index}-${workIndex}-work-title`, fieldHints.workItemTitle)} className={`${inputClass} mt-1`} />{renderHint(`${index}-${workIndex}-work-title`, fieldHints.workItemTitle)}</label>
                  ...
                  <label className="text-xs font-semibold">{uiCopy.taskDescription || 'Task description'}<textarea disabled={readOnly} value={workItem.description || ''} onChange={e => updateWorkItem(index, workIndex, { description: e.target.value })} placeholder={fieldPlaceholders.workItemDescription || 'Task description'} aria-label={uiCopy.taskDescription ? `${uiCopy.workItem || 'Work item'} ${workIndex + 1}: ${uiCopy.taskDescription}` : `Work item ${workIndex + 1} description`} aria-describedby={describedBy(`${index}-${workIndex}-work-description`, fieldHints.workItemDescription)} rows={2} className={`${inputClass} mt-1`} />{renderHint(`${index}-${workIndex}-work-description`, fieldHints.workItemDescription)}</label>
