    ...
    <section className="space-y-4 border-t border-border pt-7">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border bg-muted/20 p-4">
        <span className="text-xs font-bold uppercase text-muted-foreground">
          Fixed project budget from milestones
          ...
          <article key={milestone.id || index} className={`overflow-hidden rounded-lg border bg-card ${Object.keys(errors).some(key => key.startsWith(`${index}.`)) ? 'border-red-500/60' : 'border-border'}`}>
            ...
            {isExpanded && <div className="space-y-5 border-t border-border bg-background/40 p-4">
              <div className="grid gap-3 md:grid-cols-2">
                <label className="text-xs font-semibold">Milestone title<input data-milestone-field={`${index}.title`} disabled={readOnly} value={milestone.title || ''} onChange={e => updateMilestone(index, { title: e.target.value })} placeholder={fieldPlaceholders.milestoneTitle} aria-describedby={describedBy(`${index}-title`, fieldHints.milestoneTitle)} className={`${fieldClass('title')} mt-1`} />{renderHint(`${index}-title`, fieldHints.milestoneTitle)}{errorFor('title') && <span className="mt-1 block text-xs text-red-500">{errorFor('title')}</span>}</label>
                ...
                <label className="text-xs font-semibold">
                  ...
                  {structuredDuration && durationUnits ? (
                    <span className="mt-1 grid grid-cols-[minmax(0,1fr)_minmax(7rem,auto)] gap-2">
                      ...
                      <select disabled={readOnly} value={structuredDuration.unit} onChange={e => updateMilestone(index, { estimatedDuration: serializeStructuredDuration(structuredDuration.amount, e.target.value) })} aria-label="Duration unit" className={fieldClass('estimatedDuration')}>
                        {durationUnits.map(unit => <option key={unit.value} value={unit.value}>{unit.label}</option>)}
                      </select>
                    </span>
                  ) : (
                    <input data-milestone-field={`${index}.estimatedDuration`} disabled={readOnly} value={milestone.estimatedDuration || ''} onChange={e => updateMilestone(index, { estimatedDuration: e.target.value })} placeholder={fieldPlaceholders.duration || 'e.g. 2 weeks'} aria-describedby={describedBy(`${index}-duration`, fieldHints.duration)} className={`${fieldClass('estimatedDuration')} mt-1`} />
                  )}
              ...
              <div className="space-y-3 rounded-lg border border-border bg-card p-3">
                ...
                {milestone.workItems.map((workItem, workIndex) => <div key={workItem.id || workIndex} className="grid gap-2 rounded-lg border border-border p-3 md:grid-cols-2">
                  ...
                  <label className="text-xs font-semibold">Work item title<input disabled={readOnly} value={workItem.title || ''} onChange={e => updateWorkItem(index, workIndex, { title: e.target.value })} placeholder={fieldPlaceholders.workItemTitle || 'Work item title'} aria-label={`Work item ${workIndex + 1} title`} aria-describedby={describedBy(`${index}-${workIndex}-work-title`, fieldHints.workItemTitle)} className={`${inputClass} mt-1`} />{renderHint(`${index}-${workIndex}-work-title`, fieldHints.workItemTitle)}</label>
                  <label className="text-xs font-semibold">Estimated duration<input disabled={readOnly} value={workItem.estimatedDuration || ''} onChange={e => updateWorkItem(index, workIndex, { estimatedDuration: e.target.value })} placeholder={fieldPlaceholders.workItemDuration || 'Estimated duration'} aria-describedby={describedBy(`${index}-${workIndex}-work-duration`, fieldHints.workItemDuration)} className={`${inputClass} mt-1`} />{renderHint(`${index}-${workIndex}-work-duration`, fieldHints.workItemDuration)}</label>
