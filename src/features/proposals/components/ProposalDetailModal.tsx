...
export function ProposalDetailModal({
  ...
  return (
    ...
    >
      ...
      {/* Main Dialog Container matching ProjectReviewDialog.tsx - Increased overall modal size */}
      <div
        onClick={e => e.stopPropagation()}
        className="relative z-10 w-full max-w-6xl h-[88vh] max-h-[820px] min-h-[600px] rounded-[2rem] overflow-hidden flex flex-col lg:flex-row shadow-[0_25px_60px_-15px_rgba(0,0,0,0.3)] border border-border/50 transition-none"
        style={{ background: 'rgba(var(--background-rgb, 255,255,255), 0.95)', backdropFilter: 'blur(20px)' }}
      >
        {/* ═══ LEFT COLUMN: Candidate Hero & Proposal Context (Reduced to ~32% Width) ═══════════ */}
        <div className="w-full lg:w-1/3 p-6 lg:p-8 flex flex-col justify-between border-b lg:border-b-0 lg:border-r border-border/40 bg-surface-muted/40 relative overflow-hidden shrink-0">
          ...
          {/* Candidate Avatar Hero - Enlarged Avatar, removed outer div border wrapper */}
          <div className="relative z-10 flex flex-col items-center my-6 text-center">
            <div className="relative mb-4 flex-shrink-0 flex items-center justify-center">
              ...
              <UserAvatar
                ...
                size="xl"
                className="!w-36 !h-36 !text-4xl shadow-2xl relative z-10"
              />
            </div>
          ...
          <div className="relative z-10 rounded-2xl border border-border/60 bg-background/70 p-4 space-y-2.5 shadow-2xs">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div className="bg-surface-muted/60 p-2.5 rounded-xl border border-border/40">
                <span className="block text-[10px] font-black uppercase text-text-muted">{t('proposalReview.columns.offer')}</span>
                <strong className="text-brand font-black text-sm">{formatGigCoin(detail?.proposedBudget || activeProposal?.proposedBudget || 0)}</strong>
              </div>
              <div className="bg-surface-muted/60 p-2.5 rounded-xl border border-border/40">
                <span className="block text-[10px] font-black uppercase text-text-muted">Milestones</span>
                <strong className="text-emerald-600 dark:text-emerald-400 font-extrabold text-sm">{formatGigCoin(detailMilestoneTotal)}</strong>
              </div>
            </div>

            <div className="flex items-center justify-between text-xs text-text-muted font-medium pt-1 border-t border-border/40">
              <span className="flex items-center gap-1.5">
                <Clock size={13} className="text-indigo-400" /> Duration:
              </span>
              <strong className="text-text-primary font-bold">{detail?.proposedDuration || activeProposal?.proposedDuration || 'N/A'}</strong>
            </div>
          </div>
        ...
        {/* ═══ RIGHT COLUMN: Interactive Tab Content & Actions (Expanded to ~68% Width) ═════════ */}
        <div className="w-full lg:w-2/3 p-6 lg:p-8 bg-background flex flex-col justify-between relative min-w-0">
          ...
          {/* Top Segmented Tab Switcher Navigation - Renamed Q&A Answers to Q&A */}
                ...
                {!evalLoading && (
                  rawAnswers.length > 0 ? (
                    <div className="space-y-4">
                      <div className="flex items-center justify-between border-b border-border/60 pb-2">
                        <h4 className="text-xs font-black text-text-primary uppercase tracking-wider flex items-center gap-2">
                          ...
                          <span>Screening Questions & Answers</span>
                        </h4>
                    ...
                    <div className="rounded-2xl border border-border bg-surface-muted/20 p-12 text-center text-xs text-text-muted space-y-2">
                      ...
                      <p className="font-bold text-text-primary text-sm">No freelancer interview answers available.</p>
                    </div>
                  )
                )}
              </>
            ...
            {modalTab === 'proposalDetails' && (
              <div className="space-y-4">
                {detailLoading ? (
                  ...
                  <div className="py-12 text-center text-xs font-semibold text-text-muted">No proposal details available.</div>
                ) : (
                  <>
                    {renderDetailSection('Introduction & Overview', detail.coverLetter)}
                    {renderDetailSection('Problem Analysis', detail.analysisSummary)}
                    {renderDetailSection('Technical & Solution Approach', detail.solutionApproach)}
                    {renderDetailSection('Overall Deliverables', detail.deliverables)}
                    {renderDetailSection('Project Assumptions', detail.assumptions)}
                    {renderDetailSection('Out of Scope Items', detail.outOfScope)}

                    <section className="space-y-3 pt-2">
                      <h4 className="text-xs font-black uppercase tracking-wider text-text-muted flex items-center gap-1.5">
                        <Layers size={14} className="text-brand" />
                        Milestone Plan Breakdown
                      </h4>
                      <div className="space-y-3">
                        ...
                        )) : <p className="text-xs text-text-muted italic">Legacy proposal: no milestone plan.</p>}
                      </div>
                    </section>
                  </>
                )}
              ...
              <>
                ...
                {!evalLoading && !evalResult && (
                  <div className="rounded-2xl border border-border bg-surface-muted/20 p-12 text-center text-xs text-muted-foreground space-y-3">
                    ...
                    <div>
                      <p className="font-bold text-foreground text-sm">No AI Evaluation Interview Report available.</p>
                    ...
                    <div className="rounded-2xl border border-purple-500/25 bg-purple-500/5 p-5 space-y-4 shadow-2xs">
                      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-purple-500/15 pb-4">
                        ...
                        <div className="flex items-center gap-3.5">
                          ...
                          <div>
                            <h4 className="text-[10px] font-black uppercase tracking-wider text-text-muted">{t('proposalAnswers.overallScore')}</h4>
                            <p className="text-xs font-extrabold">{t('proposalAnswers.aiScore', { score: evalResult.score })}</p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {evalResult.recommendedHire ? (
                            <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/15 border border-emerald-500/30 px-3.5 py-1 text-xs font-black text-emerald-600 dark:text-emerald-400">
                              <CheckCircle2 size={13} /> {t('proposalAnswers.recommended')}
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-1.5 rounded-full bg-rose-500/15 border border-rose-500/30 px-3.5 py-1 text-xs font-black text-rose-500">
                              <XCircle size={13} /> {t('proposalAnswers.notRecommended')}
                            </span>
                          )}
                        </div>
                      </div>
                      ...
                      {/* Summary */}
                      <div>
                        <h4 className="text-[11px] font-black uppercase tracking-wider text-text-muted mb-1.5">{t('proposalAnswers.summary')}</h4>
                      ...
                      {(evalResult.holisticAdjustment !== 0 || evalResult.holisticAdjustmentReason) && (
                        <div className="rounded-xl bg-purple-500/10 border border-purple-500/20 p-3 text-xs space-y-1">
                          <div className="flex items-center gap-1.5">
                            <span className="font-black text-purple-700 dark:text-purple-300 uppercase tracking-wider text-[10px]">Holistic Adjustment:</span>
          ...
          {/* Bottom Actions Row - All buttons pushed to the right, removed text 'Close' button */}
          <div className="pt-4 border-t border-border/60 flex items-center justify-end gap-3 shrink-0">
            ...
            {activeId && currentStatus === ProposalStatus.Accepted && selectedJobCanNegotiate && (
              ...
              >
                <MessageSquare size={14} /> Open Negotiation Room
              </button>
            )}
          </div>
        </div>
