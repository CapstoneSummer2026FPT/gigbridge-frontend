...
describe('ClientProposalsScreen', () => {
  ...
  it('renders the AI Leaderboard and the evaluation modal with tabs', async () => {
    ...
    // Toggle view mode to AI Judging Leaderboard
    const aiJudgingBtn = await screen.findByRole('button', { name: /AI Judging Leaderboard/i });
    await user.click(aiJudgingBtn);

    // Wait for the proposal list to load
    await waitFor(() => expect(screen.getAllByText('Ada Freelancer').length).toBeGreaterThan(0));

    // Open detail modal by clicking candidate in Leaderboard
