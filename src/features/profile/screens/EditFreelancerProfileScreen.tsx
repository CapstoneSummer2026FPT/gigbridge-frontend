...
export default function EditFreelancerProfileScreen() {
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.title.length > 255) {
      newErrors.title = 'MSG01: 255 characters only, please!';
    }
    if (formData.title.trim().length === 0) {
      newErrors.title = 'MSG02: Professional title can not be all space';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (Math.random() < 0.05) { // 5% chance for demo
      ...
      setErrors({ banned: 'MSG30: This account was being banned!' });
      return;
    }
      ...
      <div className="max-w-4xl mx-auto py-6">
        ...
        {showBannedMessage && (
          <div className="edit-freelancer-profile-banned-message">
            ...
            <p className="text-sm text-red font-medium">MSG30: This account was being banned!</p>
          </div>
        )}
