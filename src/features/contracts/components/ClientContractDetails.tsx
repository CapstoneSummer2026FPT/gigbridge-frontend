...
import {
  ...
  ShieldAlert, ListChecks, Copy, Check, ChevronDown, Star, Sparkles, Eye, Mail, RefreshCw, LoaderCircle, CheckCircle, Users,
} from 'lucide-react';
              ...
              <div className="glass-card p-6 space-y-4">
                ...
                <div className="space-y-3">
                  <button
                    ...
                    className="w-full py-3 px-4 rounded-xl font-extrabold text-xs text-white transition cursor-pointer shadow-xs flex items-center justify-center gap-2 border-none"
                  ...
                  {contract.canReview && (
                    <button
                      ...
                      className="w-full py-3 px-4 bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/30 text-amber-600 dark:text-amber-400 rounded-xl font-extrabold text-xs transition cursor-pointer flex items-center justify-center gap-2"
                    >
                      <Star size={16} />
                  ...
                  {contract.hasReviewedByCurrentUser && contract.status === ContractStatus.Completed && (
                    <div className="w-full py-3 px-4 bg-emerald-500/10 border border-emerald-500/30 text-emerald-600 dark:text-emerald-400 rounded-xl font-extrabold text-xs flex items-center justify-center gap-2">
                      <CheckCircle size={16} />
                      {t('reviews.reviewed')}
                    </div>
                  )}
                  ...
                  {!isAdminOverride && activeDisputeLoading && (
                    <div className="w-full py-3 border border-border rounded-xl text-text-muted text-xs font-extrabold flex items-center justify-center gap-2">
                  ...
                  {!isAdminOverride && !activeDisputeLoading && activeDisputeError && (
                    <div className="w-full p-3 bg-rose-500/10 border border-rose-500/30 text-rose-500 rounded-xl text-xs font-extrabold space-y-2">
                      <p className="m-0">{activeDisputeError}</p>
                      <button
                        type="button"
                        onClick={onRetryDispute}
                        className="w-full py-1.5 bg-background border border-rose-500/30 rounded-lg flex items-center justify-center gap-2 cursor-pointer"
                      >
                  ...
                  {!isAdminOverride && !activeDisputeLoading && !activeDisputeError && activeDispute && (
                    <button
                      type="button"
                      onClick={() => navigate(`/disputes/${activeDispute.id}`)}
                      className="w-full py-3 px-4 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/30 text-rose-500 rounded-xl font-extrabold text-xs cursor-pointer transition flex items-center justify-center gap-2"
                    >
                      <ShieldAlert size={16} />
