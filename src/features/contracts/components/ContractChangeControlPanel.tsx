      ...
      <div className="space-y-3">
        ...
        {requests.length === 0 ? <p className="text-sm text-muted-foreground">No change requests.</p> : requests.map(request => {
          ...
          return <article key={request.changeRequestId} className="rounded-lg border border-border p-3 text-xs">
            ...
            {request.clarificationRequestNote && <p className="mt-2 rounded-xl bg-background border border-amber-500/40 text-amber-600 dark:text-amber-400 p-3 text-xs leading-relaxed"><strong>Clarification requested:</strong> {request.clarificationRequestNote}</p>}
