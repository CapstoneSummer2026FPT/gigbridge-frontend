...
export default function FreelancerPremiumScreen({ initialTab = 'overview' }: { initialTab?: Tab }) {
  ...
  return <AppLayout><main className="premium-shell">
    ...
    {loading ? <div className="premium-grid"><div className="premium-skeleton" /><div className="premium-skeleton" /></div> :
      ...
      tab === 'points' ? <section className="premium-card"><h3>{points.data?.tierName || t('freelancerPremium.eloPoints')} · {points.data?.eloPoints}</h3><p className="premium-muted">{points.data?.nextTierName ? t('freelancerPremium.pointsToTier', { count: points.data.nextTierThreshold! - points.data.eloPoints, tier: points.data.nextTierName }) : t('freelancerPremium.highestTier')}</p><div className="premium-progress"><span style={{ width: `${Math.min(100, Number(points.data?.tierProgress || 0))}%` }} /></div>{points.data?.recentTransactions?.length ? points.data.recentTransactions.map(x => <div className="premium-row" key={x.id}><div><strong>{x.reason === 5 ? t('freelancerPremium.legacyIntegrityAdjustment') : x.sourceEntityType || t('freelancerPremium.activity', { reason: x.reason })}</strong><div className="premium-muted">{new Date(x.createdAt).toLocaleString(i18n.language)}</div></div><strong style={{ color: x.pointsDelta >= 0 ? '#22c55e' : '#ef4444' }}>{x.pointsDelta > 0 ? '+' : ''}{x.pointsDelta}</strong></div>) : <p className="premium-muted">{t('freelancerPremium.noPointActivity')}</p>}</section> :
